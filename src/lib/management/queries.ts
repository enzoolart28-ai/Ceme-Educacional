...
export async function getManagementDashboard(): Promise<ManagementDashboard> {
  ...
  return {
    finance: [
      ...
      { label: "Saidas", value: cash.exits, kind: "money" },
      { label: "Solicitacoes pendentes", value: requests.pending },
      { label: "Solicitacoes aprovadas", value: requests.approved },
      { label: "Solicitacoes recusadas", value: requests.rejected },
      { label: "Caixas abertos", value: sessions.filter((s) => s.status === "open").length },
      { label: "Fechamentos com diferenca", value: sessions.filter((s) => (s.difference ?? 0) !== 0).length },
    ],
    academic: [
    ...
    pedagogical: [
      ...
      { label: "Conteudos nao publicados", value: await count("lessons", { status: "draft" }) },
    ],
    commercial: [
      ...
      { label: "Taxa de conversao", value: conversionRate, kind: "percent" },
    ...
    administrative: [
      ...
      { label: "Solicitacoes internas abertas", value: requests.pending },
      { label: "Solicitacoes atrasadas", value: 0 },
    ],
  };
}
